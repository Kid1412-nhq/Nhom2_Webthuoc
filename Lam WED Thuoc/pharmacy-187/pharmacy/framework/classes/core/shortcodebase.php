...
abstract class WPO_Shortcode_Base{
	...
	?>
			...
			<button type="button" class="btn btn-primary wpo-button-save"><?php echo __('Save', 'pharmacy'); ?></button>
			<button type="button" class="btn btn-default wpo-button-back"><?php echo __('Back to list', 'pharmacy'); ?></button>
	...
	private function renderUploader( $_id, $_name ) {
		...
		$output .= '<input id="' . $id . '" class="upload" type="text" name="'.$name.'" placeholder="' . __('No file chosen', 'pharmacy') .'" />' . "\n";
		if ( function_exists( 'wp_enqueue_media' ) ) {
			if ( ( $value == '' ) ) {
				$output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __( 'Upload', 'pharmacy' ) . '" />' . "\n";
			} else {
				$output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __( 'Remove', 'pharmacy' ) . '" />' . "\n";
			}
		} else {
			$output .= '<p><i>' . __( 'Upgrade your version of WordPress for full media support.', 'pharmacy' ) . '</i></p>';
		}
