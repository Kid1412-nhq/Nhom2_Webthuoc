...
if ( $type == 'flexslider' || $type == 'flexslider_fade' || $type == 'fading' ) {
    $type = ' wpb_flexslider flexslider_fade ';
    $flex_fx = ' data-flex_fx="fade"';
} else if ( $type == 'flexslider_slide' ) {
    $type = ' wpb_flexslider flexslider_slide ';
...
while ( $my_query->have_posts() ) {
    ...
    // if ( $thumbnail == '' ) $thumbnail = __("No Featured image set.", 'pharmacy');

    // Link logic
    if ( $link != 'link_no' ) {
        if ( $link == 'link_post' ) {
            $link_image_start = '<a class="link_image" href="'.get_permalink($post_id).'" title="'.sprintf( esc_attr__( 'Permalink to %s', 'pharmacy' ), the_title_attribute( 'echo=0' ) ).'">';
        }
    ...
    $description = '';
    if ( $slides_content != '' && $content != '' && ( $type == ' wpb_flexslider flexslider_fade ' || $type == ' wpb_flexslider flexslider_slide ' ) ) {
        $description = '<div class="flex-caption">';
        if ($slides_title==true) $description .= '<h3 class="entry-title">' . $link_image_start . $post_title . $link_image_end .'</h3>';
        $description .= $content;
        $description .= '</div>';
    }
...
else { $teasers = __("Nothing found." , 'pharmacy'); }

$css_class =  apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'wpb_gallery wpb_posts_slider clearfix'.$el_class, $this->settings['base']);

$output .= "\n\t".'<div class=" wpb_content_element '.$css_class.'">';
$output .= "\n\t\t".'<div class="widget wpb_wrapper">';
$output .= wpb_widget_title(array('title' => $title, 'extraclass' => 'widget-title visual-title'));
$output .= '<div class="wpb_gallery_slides widget-content'.$type.'" data-interval="'.$interval.'"'.$flex_fx.'>'.$teasers.'</div>';
