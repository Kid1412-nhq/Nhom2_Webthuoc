   ...
   public  function start_lvl(  &$output, $depth = 0, $args = array(), $megaconfig=null ) {
	...
	protected function renderAttrs( $menu, $args ,$level){
		if( $this->isLiveEdit ) {
			...
			$t .= ' data-backgroundcss="'.(isset($menu->megaconfig->backgroundcss)?$menu->megaconfig->backgroundcss:"").'"';
			return $t;
		}else{
			$t = sprintf( ' data-id="%s"   ', $menu->ID );
	...
	protected function genMegaMenuByConfig( $menu , $level  ){
		...
		$output = '';
			...
			if( isset($menu->megaconfig->image) &&  $menu->megaconfig->image ){
				$attrw .= 'background:url('.$menu->megaconfig->image.')' ;
				if( isset($menu->megaconfig->backgroundcss) &&  $menu->megaconfig->backgroundcss ){
					$attrw .= ' '.$menu->megaconfig->backgroundcss;
				}
				$attrw .= ';';
			}
